...
2. **Normalize** if MSB missing:
   ...
   - **How to correctly implement this sequential composition within one clock cycle:** at the start of Stage 6, copy `z_m`, `z_e`, `guard_bit`, `round_bit`, and `sticky` into local variables declared with blocking assignment (`=`), not nonblocking (`<=`). Thread the underflow-alignment, normalization, and rounding steps through these local variables using blocking assignment throughout, so each step reads the fully updated result of the step before it. Only at the very end of Stage 6 should you write the final local variable values into the actual registers (`z_m`, `z_e`, `guard_bit`, `round_bit`, `sticky`) using nonblocking assignment (`<=`). Do NOT assign intermediate results directly to `guard_bit`, `round_bit`, `sticky`, or `z_m` with nonblocking assignment partway through Stage 6 and then read them again later in the same stage — nonblocking assignments do not take effect until the next clock edge, so such a read will see the stale, pre-cycle value and produce an incorrect rounding decision.
3. **RNE rounding**:
